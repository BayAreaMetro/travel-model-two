#######################################################
### Script for summarizing Marin households ABM Output using HH weights
### Author: Binny M Paul, binny.paul@rsginc.com, Oct 2017
### Modified: Khademul Haque, khademul.haque@rsginc.com, July 2018
### Edited: BMP, Jan 2019
#######################################################

start_time <- Sys.time()

## Always load 'plyr' library before 'dplyr' due to compatibility issues
library(plyr)
library(weights)
library(reshape)
library(data.table)
...
# Parameters_File     <- "F:/Projects/Clients/mtc/updated_networks/version_12_full_run/input/visualizer/runtime/parameters.csv"
...
if (!dir.exists(ABM_SUMMARY_DIR)) {dir.create(ABM_SUMMARY_DIR)}


# geogXWalkDir    <- "F:/projects/clients/marin/data/geographicXwalk"
# geogXWalkDir    <- "E:/projects/clients/mtc/data/Trip End Geocodes"
# geogXWalkDir_SD <- "E:/projects/clients/mtc/data/Trip End Geocodes"
# SkimDir         <- "F:/Projects/Clients/mtc/updated_networks/version_11/skims"
# mazDataDir      <- "F:/Projects/Clients/mtc/updated_networks/version_11_v4_new_population/landuse"
...
unique_joint_tours <- fread(paste("jointTourData_", MAX_ITER, ".csv", sep = ""), fill=TRUE) # last probs col is all NA, so fill needed to avoid crash
...
mazData            <- read.csv(paste(LANDUSE_DIR, "maz_data_withDensity.csv", sep = "/"), as.is = T)

# write.table(wsLoc, paste("wsLocResults_full_", MAX_ITER, ".csv", sep = ""), sep = ",", row.names = F, quote = F)
# write.table(hh, paste("householdData_full_", MAX_ITER, ".csv", sep = ""), sep = ",", row.names = F, quote = F)

DST_SKM   <- fread(paste(SKIMS_DIR, "SOV_DIST_MD_HWYSKM.csv", sep = "/"), stringsAsFactors = F, header = T)
DST_SKM   <- melt(DST_SKM, id = c("DISTDA"))
colnames(DST_SKM) <- c("o", "d", "dist")

districtList         <- sort(unique(xwalk$COUNTYNAME))
SuperdistrictList    <- sort(unique(mazData$DistName)) # change by Khademul
...
wsLoc$HDISTRICT_S <- mazData$DistName[match(wsLoc$HomeMGRA, mazData$MAZ)]
wsLoc$WDISTRICT_S <- mazData$DistName[match(wsLoc$WorkLocation, mazData$MAZ)]
...
hh$COUNTYNAME <- xwalk$COUNTYNAME[match(hh$home_mgra, xwalk$MAZ)]
...
autoOwnershipCY <- count(hh, c("COUNTYNAME", "HHVEH"), "finalweight")
# autoOwnershipCY <- cast(autoOwnershipCY, COUNTYFP10~HHVEH, value = "freq", sum)
autoOwnershipCY <- cast(autoOwnershipCY, COUNTYNAME~HHVEH, value = "freq", sum)
...
tlfd_univ_S <- ddply(students[students$PersonType==3,c("HDISTRICT_S", "distbin", "finalweight")], c("HDISTRICT_S", "distbin"), summarise, univ = sum(HDISTRICT_S>0*finalweight))
...
tlfd_schl_S <- ddply(students[students$PersonType>=6,c("HDISTRICT_S", "distbin", "finalweight")], c("HDISTRICT_S", "distbin"), summarise, schl = sum(HDISTRICT_S>0*finalweight))
...
cat("\n Average distance to workplace (Total): ", weighted.mean(workers$WorkLocationDistance, workers$finalweight, na.rm = TRUE))
cat("\n Average distance to university (Total): ", weighted.mean(students$SchoolLocationDistance[students$PersonType==3], students$finalweight[students$PersonType==3], na.rm = TRUE))
cat("\n Average distance to school (Total): ", weighted.mean(students$SchoolLocationDistance[students$PersonType>=6], students$finalweight[students$PersonType>=6], na.rm = TRUE))
...
workTripLengths_S <- ddply(workers[,c("HDISTRICT_S", "WorkLocationDistance", "finalweight")], c("HDISTRICT_S"), summarise, work = weighted.mean(WorkLocationDistance, finalweight))
totalLength_S     <- data.frame("Total", weighted.mean(workers$WorkLocationDistance, workers$finalweight))
colnames(totalLength_S) <- colnames(workTripLengths_S)
workTripLengths_S <- rbind(workTripLengths_S, totalLength_S)

univTripLengths_S <- ddply(students[students$PersonType==3,c("HDISTRICT_S", "SchoolLocationDistance", "finalweight")], c("HDISTRICT_S"), summarise, univ = weighted.mean(SchoolLocationDistance, finalweight))
totalLength_S     <- data.frame("Total", weighted.mean(students$SchoolLocationDistance[students$PersonType==3], students$finalweight[students$PersonType==3]))
colnames(totalLength_S) <- colnames(univTripLengths_S)
univTripLengths_S <- rbind(univTripLengths_S, totalLength_S)

schlTripLengths_S <- ddply(students[students$PersonType>=6,c("HDISTRICT_S", "SchoolLocationDistance", "finalweight")], c("HDISTRICT_S"), summarise, schl = weighted.mean(SchoolLocationDistance, finalweight))
totalLength_S     <- data.frame("Total", weighted.mean(students$SchoolLocationDistance[students$PersonType>=6], students$finalweight[students$PersonType>=6]))
...
districtWorkers_S <- ddply(wsLoc[wsLoc$WorkLocation > 0,c("HDISTRICT_S", "finalweight")], c("HDISTRICT_S"), summarise, workers = sum(finalweight))
districtWfh_S     <- ddply(wsLoc[wsLoc$WorkLocation==99999,c("HDISTRICT_S", "WorkLocation", "finalweight")], c("HDISTRICT_S"), summarise, wfh = sum(finalweight))
wfh_summary_S     <- cbind(districtWorkers_S, districtWfh_S$wfh)
colnames(wfh_summary_S) <- c("SuperDistrict", "Workers", "WFH")
totalwfh_S        <- data.frame("Total", sum(wsLoc$WorkLocation>0*wsLoc$finalweight), sum(wsLoc$WorkLocation==99999)*wsLoc$finalweight)
...
tours$OSDIST <- mazData$DistName[match(tours$orig_mgra, mazData$MAZ)] # added by khademul
tours$DSDIST <- mazData$DistName[match(tours$dest_mgra, mazData$MAZ)] # added by khademul
...
tours$TOURMODE[tours$tour_mode %in% c(3,4,5)]       <- 2 # SR2
...
tours$TOURMODE[tours$tour_mode %in% c(13)]          <- 8 # KNR_PERS
tours$TOURMODE[tours$tour_mode %in% c(14)]          <- 9 # KNR_TNC
tours$TOURMODE[tours$tour_mode %in% c(17)]          <- 10 # SCHBUS
tours$TOURMODE[tours$tour_mode %in% c(15)]          <- 11 # TAXI
tours$TOURMODE[tours$tour_mode %in% c(16)]          <- 12 # TNC
...
unique_joint_tours$OSDIST <- mazData$DistName[match(unique_joint_tours$orig_mgra, mazData$MAZ)] # added by khademul
unique_joint_tours$DSDIST <- mazData$DistName[match(unique_joint_tours$dest_mgra, mazData$MAZ)] # added by khademul
#compute duration
...
unique_joint_tours$TOURMODE[unique_joint_tours$tour_mode %in% c(3,4,5)]       <- 2 # SR2
...
unique_joint_tours$TOURMODE[unique_joint_tours$tour_mode %in% c(13)]          <- 8 # KNR_PERS
unique_joint_tours$TOURMODE[unique_joint_tours$tour_mode %in% c(14)]          <- 9 # KNR_TNC
unique_joint_tours$TOURMODE[unique_joint_tours$tour_mode %in% c(17)]          <- 10 # SCHBUS
unique_joint_tours$TOURMODE[unique_joint_tours$tour_mode %in% c(15)]          <- 11 # TAXI
unique_joint_tours$TOURMODE[unique_joint_tours$tour_mode %in% c(16)]          <- 12 # TNC
...
trips$TOURMODE[trips$tour_mode %in% c(3,4,5)]       <- 2 # SR2
...
trips$TOURMODE[trips$tour_mode %in% c(13)]          <- 8 # KNR_PERS
trips$TOURMODE[trips$tour_mode %in% c(14)]          <- 9 # KNR_TNC
trips$TOURMODE[trips$tour_mode %in% c(17)]          <- 10 # SCHBUS
trips$TOURMODE[trips$tour_mode %in% c(15)]          <- 11 # TAXI
trips$TOURMODE[trips$tour_mode %in% c(16)]          <- 12 # TNC

trips$TRIPMODE[trips$trip_mode %in% c(1,2)]         <- 1 # SOV 
trips$TRIPMODE[trips$trip_mode %in% c(3,4,5)]       <- 2 # SR2
...
trips$TRIPMODE[trips$trip_mode %in% c(13)]          <- 8 # KNR_PERS
trips$TRIPMODE[trips$trip_mode %in% c(14)]          <- 9 # KNR_TNC
trips$TRIPMODE[trips$trip_mode %in% c(17)]          <- 10 # SCHBUS
trips$TRIPMODE[trips$trip_mode %in% c(15)]          <- 11 # TAXI
trips$TRIPMODE[trips$trip_mode %in% c(16)]          <- 12 # TNC
...
#code aggregate tour purposes for comparison with OBS (added by Khademul)
trips$AGGTOURPURP[trips$tour_purpose=="Home"] <- "Home"
trips$AGGTOURPURP[trips$tour_purpose=="Work"] <- "work"
trips$AGGTOURPURP[trips$tour_purpose=="Work-Based" | trips$tour_purpose=="work related"] <- "at-work"
trips$AGGTOURPURP[trips$tour_purpose=="School"] <- "school"
trips$AGGTOURPURP[trips$tour_purpose=="University"] <- "university"
trips$AGGTOURPURP[trips$tour_purpose=="Shop" | trips$tour_purpose=="Maintenance" | trips$tour_purpose=="Escort"] <- "maintenance"
trips$AGGTOURPURP[trips$tour_purpose=="Eating Out" | trips$tour_purpose=="Visiting" | trips$tour_purpose=="Discretionary"] <- "discretionary"
...
trips$OSDIST <- mazData$DistName[match(trips$orig_mgra, mazData$MAZ)] # added by khademul
trips$DSDIST <- mazData$DistName[match(trips$dest_mgra, mazData$MAZ)] # added by khademul
...
jtrips$TOURMODE[jtrips$tour_mode %in% c(1,2)]         <- 1 # SOV 
jtrips$TOURMODE[jtrips$tour_mode %in% c(3,4,5)]       <- 2 # SR2
jtrips$TOURMODE[jtrips$tour_mode %in% c(6,7,8)]       <- 3 # SR3
jtrips$TOURMODE[jtrips$tour_mode %in% c(9)]           <- 4 # Walk
jtrips$TOURMODE[jtrips$tour_mode %in% c(10)]          <- 5 # Bike
jtrips$TOURMODE[jtrips$tour_mode %in% c(11)]          <- 6 # WT
jtrips$TOURMODE[jtrips$tour_mode %in% c(12)]          <- 7 # PNR
jtrips$TOURMODE[jtrips$tour_mode %in% c(13)]          <- 8 # KNR_PERS
jtrips$TOURMODE[jtrips$tour_mode %in% c(14)]          <- 9 # KNR_TNC
jtrips$TOURMODE[jtrips$tour_mode %in% c(17)]          <- 10 # SCHBUS
jtrips$TOURMODE[jtrips$tour_mode %in% c(15)]          <- 11 # TAXI
jtrips$TOURMODE[jtrips$tour_mode %in% c(16)]          <- 12 # TNC
...
jtrips$TRIPMODE[jtrips$trip_mode %in% c(1,2)]         <- 1 # SOV 
jtrips$TRIPMODE[jtrips$trip_mode %in% c(3,4,5)]       <- 2 # SR2
jtrips$TRIPMODE[jtrips$trip_mode %in% c(6,7,8)]       <- 3 # SR3
jtrips$TRIPMODE[jtrips$trip_mode %in% c(9)]           <- 4 # Walk
jtrips$TRIPMODE[jtrips$trip_mode %in% c(10)]          <- 5 # Bike
jtrips$TRIPMODE[jtrips$trip_mode %in% c(11)]          <- 6 # WT
jtrips$TRIPMODE[jtrips$trip_mode %in% c(12)]          <- 7 # PNR
jtrips$TRIPMODE[jtrips$trip_mode %in% c(13)]          <- 8 # KNR_PERS
jtrips$TRIPMODE[jtrips$trip_mode %in% c(14)]          <- 9 # KNR_TNC
jtrips$TRIPMODE[jtrips$trip_mode %in% c(17)]          <- 10 # SCHBUS
jtrips$TRIPMODE[jtrips$trip_mode %in% c(15)]          <- 11 # TAXI
jtrips$TRIPMODE[jtrips$trip_mode %in% c(16)]          <- 12 # TNC
...
jtrips$OSDIST <- mazData$DistName[match(jtrips$orig_mgra, mazData$MAZ)] # added by khademul
jtrips$DSDIST <- mazData$DistName[match(jtrips$dest_mgra, mazData$MAZ)] # added by khademul
...
tmode1_as0 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==1 & tours$AUTOSUFF==0], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==1 & tours$AUTOSUFF==0])
tmode2_as0 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==2 & tours$AUTOSUFF==0], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==2 & tours$AUTOSUFF==0])
tmode3_as0 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==3 & tours$AUTOSUFF==0], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==3 & tours$AUTOSUFF==0])
tmode4_as0 <- wtd.hist(tours$TOURMODE[tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$AUTOSUFF==0], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$AUTOSUFF==0])
tmode5_as0 <- wtd.hist(tours$TOURMODE[tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$AUTOSUFF==0], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$AUTOSUFF==0])
tmode6_as0 <- wtd.hist(unique_joint_tours$TOURMODE[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6 & unique_joint_tours$AUTOSUFF==0], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$numberhh_wgt[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6 & unique_joint_tours$AUTOSUFF==0])
tmode7_as0 <- wtd.hist(unique_joint_tours$TOURMODE[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9 & unique_joint_tours$AUTOSUFF==0], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$numberhh_wgt[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9 & unique_joint_tours$AUTOSUFF==0])
tmode8_as0 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==10 & tours$AUTOSUFF==0], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==10 & tours$AUTOSUFF==0])
...
tmode1_as1 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==1 & tours$AUTOSUFF==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==1 & tours$AUTOSUFF==1])
tmode2_as1 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==2 & tours$AUTOSUFF==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==2 & tours$AUTOSUFF==1])
tmode3_as1 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==3 & tours$AUTOSUFF==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==3 & tours$AUTOSUFF==1])
tmode4_as1 <- wtd.hist(tours$TOURMODE[tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$AUTOSUFF==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$AUTOSUFF==1])
tmode5_as1 <- wtd.hist(tours$TOURMODE[tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$AUTOSUFF==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$AUTOSUFF==1])
tmode6_as1 <- wtd.hist(unique_joint_tours$TOURMODE[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6 & unique_joint_tours$AUTOSUFF==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$numberhh_wgt[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6 & unique_joint_tours$AUTOSUFF==1])
tmode7_as1 <- wtd.hist(unique_joint_tours$TOURMODE[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9 & unique_joint_tours$AUTOSUFF==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$numberhh_wgt[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9 & unique_joint_tours$AUTOSUFF==1])
tmode8_as1 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==10 & tours$AUTOSUFF==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==10 & tours$AUTOSUFF==1])
...
tmode1_as2 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==1 & tours$AUTOSUFF==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==1 & tours$AUTOSUFF==2])
tmode2_as2 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==2 & tours$AUTOSUFF==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==2 & tours$AUTOSUFF==2])
tmode3_as2 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==3 & tours$AUTOSUFF==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==3 & tours$AUTOSUFF==2])
tmode4_as2 <- wtd.hist(tours$TOURMODE[tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$AUTOSUFF==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$AUTOSUFF==2])
tmode5_as2 <- wtd.hist(tours$TOURMODE[tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$AUTOSUFF==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$AUTOSUFF==2])
tmode6_as2 <- wtd.hist(unique_joint_tours$TOURMODE[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6 & unique_joint_tours$AUTOSUFF==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$numberhh_wgt[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6 & unique_joint_tours$AUTOSUFF==2])
tmode7_as2 <- wtd.hist(unique_joint_tours$TOURMODE[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9 & unique_joint_tours$AUTOSUFF==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$numberhh_wgt[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9 & unique_joint_tours$AUTOSUFF==2])
tmode8_as2 <- wtd.hist(tours$TOURMODE[tours$TOURPURP==10 & tours$AUTOSUFF==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==10 & tours$AUTOSUFF==2])
...
#Trip Mode Summary
#Work
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==3], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==4], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==5], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==6], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==7], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==8], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==9], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==9])
tripmode10 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==10], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==10])
tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==11], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==11])
tripmode12 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==12], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==12])
...
tripModeProfile <- data.frame(tripmode1$counts, tripmode2$counts, tripmode3$counts, tripmode4$counts,
                              ...
                              tripmode9$counts, tripmode10$counts, tripmode11$counts, tripmode12$counts)
colnames(tripModeProfile) <- c("tourmode1", "tourmode2", "tourmode3", "tourmode4", "tourmode5", "tourmode6",
                               "tourmode7", "tourmode8", "tourmode9", "tourmode10", "tourmode11", "tourmode12")
...
#University
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==3], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==4], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==5], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==6], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==7], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==8], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==9], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==9])
tripmode10 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==10], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==10])
tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==11], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==11])
tripmode12 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==12], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==12])
...
#School
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==3], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==4], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==5], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==6], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==7], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==8], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==9], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==9])
tripmode10 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==10], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==10])
tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==11], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==11])
tripmode12 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==12], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==12])
...
#iMain
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==3], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==4], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==5], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==6], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==7], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==8], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==9], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==9])
tripmode10 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==10], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==10])
tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==11], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==11])
tripmode12 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==12], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==12])
...
#iDisc
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==3], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==4], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==5], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==6], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==7], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==8], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==9], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==9])
tripmode10 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==10], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==10])
tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==11], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==11])
tripmode12 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==12], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==12])
...
#jMain
tripmode1 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==1])
tripmode2 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==2])
tripmode3 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==3], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==3])
tripmode4 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==4], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==4])
tripmode5 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==5], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==5])
tripmode6 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==6], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==6])
tripmode7 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==7], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==7])
tripmode8 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==8], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==8])
tripmode9 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==9], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==9])
tripmode10 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==10], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==10])
tripmode11 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==11], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==11])
tripmode12 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==12], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==12])
...
#jDisc
tripmode1 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==1])
tripmode2 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==2])
tripmode3 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==3], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==3])
tripmode4 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==4], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==4])
tripmode5 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==5], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==5])
tripmode6 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==6], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==6])
tripmode7 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==7], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==7])
tripmode8 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==8], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==8])
tripmode9 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==9], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==9])
tripmode10 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==10], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==10])
tripmode11 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==11], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==11])
tripmode12 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==12], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==12])
...
#At work
tripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==1])
tripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==2])
tripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==3], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==3])
tripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==4], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==4])
tripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==5], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==5])
tripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==6], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==6])
tripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==7], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==7])
tripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==8], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==8])
tripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==9], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==9])
tripmode10 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==10], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==10])
tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==11], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==11])
tripmode12 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==12], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==12])
...
#iTotal
itripmode1 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==1])
itripmode2 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==2])
itripmode3 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==3], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==3])
itripmode4 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==4], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==4])
itripmode5 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==5], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==5])
itripmode6 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==6], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==6])
itripmode7 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==7], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==7])
itripmode8 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==8], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==8])
itripmode9 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==9], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==9])
itripmode10 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==10], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==10])
itripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==11], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==11])
itripmode12 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==12], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==12])

#jTotal
jtripmode1 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==1], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==1])
jtripmode2 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==2], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==2])
jtripmode3 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==3], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==3])
jtripmode4 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==4], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==4])
jtripmode5 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==5], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==5])
jtripmode6 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==6], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==6])
jtripmode7 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==7], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==7])
jtripmode8 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==8], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==8])
jtripmode9 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==9], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==9])
jtripmode10 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==10], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==10])
jtripmode11 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==11], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==11])
jtripmode12 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==12], breaks = seq(1,13, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==12])
...
temp$tourmode[temp$tourmode=="tourmode11"] <- 'Taxi'
temp$tourmode[temp$tourmode=="tourmode12"] <- 'TNC'
...
trips_sample <- rbind(trips[,c("OCOUNTY", "DCOUNTY", "OSDIST", "DSDIST", "TRIPMODE", "tour_purpose", "num_participants", "finalweight")], 
                      jtrips[,c("OCOUNTY", "DCOUNTY", "OSDIST", "DSDIST", "TRIPMODE", "tour_purpose", "num_participants", "finalweight")])
...
trips_sample <- trips_sample[,c("OCOUNTY", "DCOUNTY", "OSDIST", "DSDIST", "trip_mode", "tour_purpose", "num_participants", "finalweight")]
...
write.table(trips_flow, "trips_flow.csv", sep = ",", row.names = F)

# added by khademul
trips_flow_S <- trips_sample[, .(count = sum(finalweight)), by = list(OSDIST, DSDIST, trip_mode, tour_purpose)]
...
write.table(trips_flow_S, "trips_flow_S.csv", sep = ",", row.names = F)
...
trips_transit <- data.table(trips_transit[,c("OCOUNTY", "DCOUNTY", "OSDIST", "DSDIST", "access_mode", "AGGTOURPURP", "finalweight")])

trips_transit_summary <- trips_transit[, .(count = sum(finalweight)), by = list(OCOUNTY, DCOUNTY, access_mode, AGGTOURPURP)]
...
trips_transit_summary <- rbind(trips_transit_summary, trips_transit_summary_total[,c("OCOUNTY", "DCOUNTY", "access_mode", "AGGTOURPURP", "count")])

trips_transit_summary_total <- data.table(trips_transit_summary[,c("OCOUNTY", "DCOUNTY", "AGGTOURPURP", "count")])
trips_transit_summary_total <- trips_transit_summary_total[, (tot = sum(count)), by = list(OCOUNTY, DCOUNTY, AGGTOURPURP)]
trips_transit_summary_total$access_mode <- "Total"
names(trips_transit_summary_total)[names(trips_transit_summary_total) == "V1"] <- "count"
trips_transit_summary <- rbind(trips_transit_summary, trips_transit_summary_total[,c("OCOUNTY", "DCOUNTY", "access_mode", "AGGTOURPURP", "count")])

write.table(trips_transit_summary, "trips_transit_summary.csv", sep = ",", row.names = F)

# added by khademul
trips_transit_summary_S <- trips_transit[, .(count = sum(finalweight)), by = list(OSDIST, DSDIST, access_mode, AGGTOURPURP)]
...
trips_transit_summary_total_S$AGGTOURPURP <- "Total"
names(trips_transit_summary_total_S)[names(trips_transit_summary_total_S) == "V1"] <- "count"
trips_transit_summary_S <- rbind(trips_transit_summary_S, trips_transit_summary_total_S[,c("OSDIST", "DSDIST", "access_mode", "AGGTOURPURP", "count")])

trips_transit_summary_total_S <- data.table(trips_transit_summary_S[,c("OSDIST", "DSDIST", "AGGTOURPURP", "count")])
trips_transit_summary_total_S <- trips_transit_summary_total_S[, (tot = sum(count)), by = list(OSDIST, DSDIST, AGGTOURPURP)]
trips_transit_summary_total_S$access_mode <- "Total"
names(trips_transit_summary_total_S)[names(trips_transit_summary_total_S) == "V1"] <- "count"
trips_transit_summary_S <- rbind(trips_transit_summary_S, trips_transit_summary_total_S[,c("OSDIST", "DSDIST", "access_mode", "AGGTOURPURP", "count")])

write.table(trips_transit_summary_S, "trips_transit_summary_S.csv", sep = ",", row.names = F)
...
percent_pass_holders <- ddply(per[,c("PERTYPE", "transitPass_choice", "finalweight")], c("PERTYPE"), summarise, percent_pass_holders = weighted.mean(transitPass_choice, finalweight)*100)
...
percent_with_subsidy <- ddply(per[,c("PERTYPE", "transitSubsidy_choice", "finalweight")], c("PERTYPE"), summarise, percent_with_subsidy = weighted.mean(transitSubsidy_choice, finalweight)*100)
write.table(percent_with_subsidy, "pecentWithSubsidy.csv", sep=",", row.names=F)

# Average subsidy amount if the subsidy exists
average_subsidy_amount <- ddply(per[per$transitSubsidy_choice == 1,c("PERTYPE", "transitSubsidy_percent", "finalweight")], c("PERTYPE"), summarise, average_subsidy_amount = weighted.mean(transitSubsidy_percent, finalweight)*100)
write.table(average_subsidy_amount, "averageSubsidyAmount.csv", sep=",", row.names=F)

# Subsidy Distribution by Person Type
subsidyfreq1 <- wtd.hist(per$transitSubsidy_percent[per$PERTYPE==1 & per$transitSubsidy_choice==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==1 & per$transitSubsidy_choice==1])
subsidyfreq2 <- wtd.hist(per$transitSubsidy_percent[per$PERTYPE==2 & per$transitSubsidy_choice==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==2 & per$transitSubsidy_choice==1])
subsidyfreq3 <- wtd.hist(per$transitSubsidy_percent[per$PERTYPE==3 & per$transitSubsidy_choice==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==3 & per$transitSubsidy_choice==1])
subsidyfreq4 <- wtd.hist(per$transitSubsidy_percent[per$PERTYPE==4 & per$transitSubsidy_choice==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==4 & per$transitSubsidy_choice==1])
subsidyfreq5 <- wtd.hist(per$transitSubsidy_percent[per$PERTYPE==5 & per$transitSubsidy_choice==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==5 & per$transitSubsidy_choice==1])
subsidyfreq6 <- wtd.hist(per$transitSubsidy_percent[per$PERTYPE==6 & per$transitSubsidy_choice==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==6 & per$transitSubsidy_choice==1])
subsidyfreq7 <- wtd.hist(per$transitSubsidy_percent[per$PERTYPE==7 & per$transitSubsidy_choice==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==7 & per$transitSubsidy_choice==1])
subsidyfreq8 <- wtd.hist(per$transitSubsidy_percent[per$PERTYPE==8 & per$transitSubsidy_choice==1], breaks = c(seq(0,1, by=.1), 9999), freq = NULL, right=TRUE, weight = per$finalweight[per$PERTYPE==8 & per$transitSubsidy_choice==1])
