...
# Directories
WD                   <- "F:/Projects/Clients/mtc/MTC_Visualizer/data/CHTS_Summaries"
...
SkimDir              <- "F:/Projects/Clients/mtc/updated_networks/version_08_v2/skims"
...
mazDataDir           <- "F:/Projects/Clients/mtc/updated_networks/version_08_v2/landuse"
...
# Read taz_tm2 to SD34 crosswalk
taz_SD               <- read.csv(file.path(SD_Dir,'tazs_TM2_v2_2_SUPERD.csv'), as.is = T)
colnames(taz_SD)     <- c("ZONE", "SD", "SD_NAME")
...
hh$HHMAZ[hh$SAMPN==1396611] <- 37041
hh$HHMAZ[hh$SAMPN==2523439] <- 38471
hh$HHMAZ[hh$SAMPN==2565278] <- 6894
hh$HHMAZ[hh$SAMPN==2644246] <- 11161
hh$HHMAZ[hh$SAMPN==2664190] <- 6849
hh$HHMAZ[hh$SAMPN==3019550] <- 10571
hh$HHMAZ[hh$SAMPN==7020181] <- 8432
...
trips$ORIG_MAZ_ORIGINAL <- plloc$MAZ_V10[match(paste(trips$HH_ID, trips$PER_ID, trips$ORIG_PLACENO, sep = "-"), 
                                      paste(plloc$SAMPN, plloc$PERNO, plloc$PLANO, sep = "-"))]
trips$ORIG_MAZ <- xwalk$MAZ[match(trips$ORIG_MAZ_ORIGINAL, xwalk$MAZ_ORIGINAL)]
trips$ORIG_TAZ_ORIGINAL <- xwalk$TAZ_ORIGINAL[match(trips$ORIG_MAZ, xwalk$MAZ)]
trips$ORIG_TAZ <- xwalk$TAZ[match(trips$ORIG_MAZ, xwalk$MAZ)]
trips$ORIG_MAZ[is.na(trips$ORIG_MAZ)] <- 0
trips$ORIG_MAZ_ORIGINAL[is.na(trips$ORIG_MAZ_ORIGINAL)] <- 0
trips$ORIG_TAZ[is.na(trips$ORIG_TAZ)] <- 0
trips$ORIG_TAZ_ORIGINAL[is.na(trips$ORIG_TAZ_ORIGINAL)] <- 0

trips$DEST_MAZ_ORIGINAL <- plloc$MAZ_V10[match(paste(trips$HH_ID, trips$PER_ID, trips$DEST_PLACENO, sep = "-"), 
                                      paste(plloc$SAMPN, plloc$PERNO, plloc$PLANO, sep = "-"))]
trips$DEST_MAZ <- xwalk$MAZ[match(trips$DEST_MAZ_ORIGINAL, xwalk$MAZ_ORIGINAL)]
trips$DEST_TAZ_ORIGINAL <- xwalk$TAZ_ORIGINAL[match(trips$DEST_MAZ, xwalk$MAZ)]
trips$DEST_TAZ <- xwalk$TAZ[match(trips$DEST_MAZ, xwalk$MAZ)]
trips$DEST_MAZ[is.na(trips$DEST_MAZ)] <- 0
trips$DEST_MAZ_ORIGINAL[is.na(trips$DEST_MAZ_ORIGINAL)] <- 0
trips$DEST_TAZ[is.na(trips$DEST_TAZ)] <- 0
trips$DEST_TAZ_ORIGINAL[is.na(trips$DEST_TAZ_ORIGINAL)] <- 0
...
trips$Orig_SD34 <- taz_SD$SD[match(trips$ORIG_TAZ_ORIGINAL, taz_SD$ZONE)]
trips$Dest_SD34 <- taz_SD$SD[match(trips$DEST_TAZ_ORIGINAL, taz_SD$ZONE)]
trips$Orig_SD34[is.na(trips$Orig_SD34)] <- 0
trips$Dest_SD34[is.na(trips$Dest_SD34)] <- 0
trips$Orig_SD34_NAME <- taz_SD$SD_NAME[match(trips$ORIG_TAZ_ORIGINAL, taz_SD$ZONE)]
trips$Dest_SD34_NAME <- taz_SD$SD_NAME[match(trips$DEST_TAZ_ORIGINAL, taz_SD$ZONE)]
trips$Orig_SD34_NAME[is.na(trips$Orig_SD34_NAME)] <- ""
trips$Dest_SD34_NAME[is.na(trips$Dest_SD34_NAME)] <- ""

tours$ORIG_MAZ_ORIGINAL <- plloc$MAZ_V10[match(paste(tours$HH_ID, tours$PER_ID, tours$ORIG_PLACENO, sep = "-"), 
                                      paste(plloc$SAMPN, plloc$PERNO, plloc$PLANO, sep = "-"))]
tours$ORIG_MAZ <- xwalk$MAZ[match(tours$ORIG_MAZ_ORIGINAL, xwalk$MAZ_ORIGINAL)]
tours$ORIG_TAZ_ORIGINAL <- xwalk$TAZ_ORIGINAL[match(tours$ORIG_MAZ, xwalk$MAZ)]
tours$ORIG_TAZ <- xwalk$TAZ[match(tours$ORIG_MAZ, xwalk$MAZ)]
tours$ORIG_MAZ[is.na(tours$ORIG_MAZ)] <- 0
tours$ORIG_MAZ_ORIGINAL[is.na(tours$ORIG_MAZ_ORIGINAL)] <- 0
tours$ORIG_TAZ[is.na(tours$ORIG_TAZ)] <- 0
tours$ORIG_TAZ_ORIGINAL[is.na(tours$ORIG_TAZ_ORIGINAL)] <- 0

tours$DEST_MAZ_ORIGINAL <- plloc$MAZ_V10[match(paste(tours$HH_ID, tours$PER_ID, tours$DEST_PLACENO, sep = "-"), 
                                      paste(plloc$SAMPN, plloc$PERNO, plloc$PLANO, sep = "-"))]
tours$DEST_MAZ <- xwalk$MAZ[match(tours$DEST_MAZ_ORIGINAL, xwalk$MAZ_ORIGINAL)]
tours$DEST_TAZ_ORIGINAL <- xwalk$TAZ_ORIGINAL[match(tours$DEST_MAZ, xwalk$MAZ)]
tours$DEST_TAZ <- xwalk$TAZ[match(tours$DEST_MAZ, xwalk$MAZ)]
tours$DEST_MAZ[is.na(tours$DEST_MAZ)] <- 0
tours$DEST_MAZ_ORIGINAL[is.na(tours$DEST_MAZ_ORIGINAL)] <- 0
tours$DEST_TAZ[is.na(tours$DEST_TAZ)] <- 0
tours$DEST_TAZ_ORIGINAL[is.na(tours$DEST_TAZ_ORIGINAL)] <- 0

tours$Orig_SD34 <- taz_SD$SD[match(tours$ORIG_TAZ_ORIGINAL, taz_SD$ZONE)]
tours$Dest_SD34 <- taz_SD$SD[match(tours$DEST_TAZ_ORIGINAL, taz_SD$ZONE)]
tours$Orig_SD34[is.na(tours$Orig_SD34)] <- 0
tours$Dest_SD34[is.na(tours$Dest_SD34)] <- 0
tours$Orig_SD34_NAME <- taz_SD$SD_NAME[match(tours$ORIG_TAZ_ORIGINAL, taz_SD$ZONE)]
tours$Dest_SD34_NAME <- taz_SD$SD_NAME[match(tours$DEST_TAZ_ORIGINAL, taz_SD$ZONE)]
tours$Orig_SD34_NAME[is.na(tours$Orig_SD34_NAME)] <- ""
tours$Dest_SD34_NAME[is.na(tours$Dest_SD34_NAME)] <- ""
...
workers <- per[per$WTAZ>0 & per$PERTYPE<=3 & per$empl_loc_type!=2, c("SAMPN", "PERNO", "HHTAZ", "WTAZ", "empl_loc_type","PERTYPE", "HDISTRICT", "WDISTRICT", "finalweight")]
...
tmode6_as0 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==0])
tmode7_as0 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==0])
...
tmode6_as1 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==1], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==1])
tmode7_as1 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==1], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==1])
...
tmode6_as2 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==2], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==2])
tmode7_as2 <- wtd.hist(tours$TOURMODE[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==2], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[!is.na(tours$TOURMODE) & tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1 & tours$AUTOSUFF==2])
...
# District X District Tour Summaries [Person Tours]
#-----------------------------------------------------

# Code Agg tour purpose
tours$TOURPURP_AGG[tours$TOURPURP==1 & tours$IS_SUBTOUR == 0] <- "WORK"
tours$TOURPURP_AGG[tours$TOURPURP==2 & tours$IS_SUBTOUR == 0] <- "UNIV"
tours$TOURPURP_AGG[tours$TOURPURP==3 & tours$IS_SUBTOUR == 0] <- "SCHL"
tours$TOURPURP_AGG[tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==0] <- "iMAIN"
tours$TOURPURP_AGG[tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==0] <- "iDISC"
tours$TOURPURP_AGG[tours$TOURPURP>=4 & tours$TOURPURP<=6 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1] <- "jMAIN"
tours$TOURPURP_AGG[tours$TOURPURP>=7 & tours$TOURPURP<=9 & tours$IS_SUBTOUR == 0 & tours$FULLY_JOINT==1] <- "jDISC"
tours$TOURPURP_AGG[tours$IS_SUBTOUR==1] <- "ATWRK"

PURP_AGG_LIST <- c("WORK", "UNIV","SCHL","iMAIN","iDISC","jMAIN","jDISC","ATWRK")

district_summary <- data.frame()
for (i in PURP_AGG_LIST){
  t <- xtabs(finalweight~TOURMODE+Orig_SD34_NAME+Dest_SD34_NAME, data = tours[tours$TOURPURP_AGG==i & tours$TOURMODE<=3 & tours$Orig_SD34>0 & tours$Dest_SD34>0,])
  t <- data.frame(t)
  t$purpose <- i
  district_summary <- rbind(district_summary,t)
}
colnames(district_summary) <- c("TOURMODE", "Orig_SD_NAME", "Dest_SD_NAME", "tours", "tourPurpose")

write.csv(district_summary, "auto_tours_district_input.csv", row.names = F)
...
#jMain                                                        
tripmode1 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==1 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==1 & trips$IS_SUBTOUR==0])
tripmode2 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==2 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==2 & trips$IS_SUBTOUR==0])
tripmode3 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==3 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==3 & trips$IS_SUBTOUR==0])
tripmode4 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==4 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==4 & trips$IS_SUBTOUR==0])
tripmode5 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==5 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==5 & trips$IS_SUBTOUR==0])
tripmode6 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==6 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==6 & trips$IS_SUBTOUR==0])
tripmode7 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==7 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==7 & trips$IS_SUBTOUR==0])
tripmode8 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==8 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==8 & trips$IS_SUBTOUR==0])
tripmode9 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==9 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$FULLY_JOINT==1 & trips$TOURMODE==9 & trips$IS_SUBTOUR==0])
...
#jDisc                                                        
tripmode1 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==1 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==1 & trips$IS_SUBTOUR==0])
tripmode2 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==2 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==2 & trips$IS_SUBTOUR==0])
tripmode3 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==3 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==3 & trips$IS_SUBTOUR==0])
tripmode4 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==4 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==4 & trips$IS_SUBTOUR==0])
tripmode5 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==5 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==5 & trips$IS_SUBTOUR==0])
tripmode6 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==6 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==6 & trips$IS_SUBTOUR==0])
tripmode7 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==7 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==7 & trips$IS_SUBTOUR==0])
tripmode8 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==8 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==8 & trips$IS_SUBTOUR==0])
tripmode9 <- wtd.hist(trips$TRIPMODE[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==9 & trips$IS_SUBTOUR==0], breaks = seq(1,12, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[!is.na(trips$TRIPMODE) & trips$TRIPMODE>0  & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$FULLY_JOINT==1 & trips$TOURMODE==9 & trips$IS_SUBTOUR==0])
...
# District X District Trip Summaries [Person Trips]
#-----------------------------------------------------

# Code Agg tour purpose
trips$TOURPURP_AGG[trips$TOURPURP==1 & trips$IS_SUBTOUR == 0] <- "WORK"
trips$TOURPURP_AGG[trips$TOURPURP==2 & trips$IS_SUBTOUR == 0] <- "UNIV"
trips$TOURPURP_AGG[trips$TOURPURP==3 & trips$IS_SUBTOUR == 0] <- "SCHL"
trips$TOURPURP_AGG[trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$IS_SUBTOUR == 0 & trips$FULLY_JOINT==0] <- "iMAIN"
trips$TOURPURP_AGG[trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$IS_SUBTOUR == 0 & trips$FULLY_JOINT==0] <- "iDISC"
trips$TOURPURP_AGG[trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$IS_SUBTOUR == 0 & trips$FULLY_JOINT==1] <- "jMAIN"
trips$TOURPURP_AGG[trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$IS_SUBTOUR == 0 & trips$FULLY_JOINT==1] <- "jDISC"
trips$TOURPURP_AGG[trips$IS_SUBTOUR==1] <- "ATWRK"

PURP_AGG_LIST <- c("WORK", "UNIV","SCHL","iMAIN","iDISC","jMAIN","jDISC","ATWRK")

district_summary <- data.frame()
for (i in PURP_AGG_LIST){
  t <- xtabs(finalweight~TRIPMODE+Orig_SD34_NAME+Dest_SD34_NAME, data = trips[trips$TOURPURP_AGG==i & trips$TRIPMODE<=3 & trips$TRIPMODE>0 & trips$Orig_SD34>0 & trips$Dest_SD34>0,])
  t <- data.frame(t)
  t$purpose <- i
  district_summary <- rbind(district_summary,t)
}
colnames(district_summary) <- c("TRIPMODE", "Orig_SD_NAME", "Dest_SD_NAME", "trips", "tourPurpose")

write.csv(district_summary, "auto_trips_district_input.csv", row.names = F)
...
total_stops <- sum(stops$finalweight[stops$TOURPURP %in% c(1,2,3)]) + 
  sum(stops$finalweight[stops$TOURPURP %in% c(4,5,6,7,8,9) & stops$FULLY_JOINT==0 & stops$IS_SUBTOUR==0]) + 
  sum(stops$finalweight[stops$TOURPURP %in% c(4,5,6,7,8,9) & stops$FULLY_JOINT==1 & stops$IS_SUBTOUR==0]) + 
  sum(stops$finalweight[stops$IS_SUBTOUR==1])
total_trips <- sum(trips$finalweight[trips$TOURPURP %in% c(1,2,3)]) + 
  sum(trips$finalweight[trips$TOURPURP %in% c(4,5,6,7,8,9) & trips$FULLY_JOINT==0 & trips$IS_SUBTOUR==0]) + 
  sum(trips$finalweight[trips$TOURPURP %in% c(4,5,6,7,8,9) & trips$FULLY_JOINT==1 & trips$IS_SUBTOUR==0]) + 
  sum(trips$finalweight[trips$IS_SUBTOUR==1])
total_tours <- sum(tours$finalweight[tours$TOURPURP %in% c(1,2,3)]) + 
  sum(tours$finalweight[tours$TOURPURP %in% c(4,5,6,7,8,9) & tours$FULLY_JOINT==0 & tours$IS_SUBTOUR==0]) + 
  sum(tours$finalweight[tours$TOURPURP %in% c(4,5,6,7,8,9) & tours$FULLY_JOINT==1 & tours$IS_SUBTOUR==0]) + 
  sum(tours$finalweight[tours$IS_SUBTOUR==1])
