...
zone_seq_file           = os.path.join('hwy',   'mtc_final_network_zone_seq.csv')

#output file
tap_lines_file          = os.path.join('trn',   'tapLines.csv')
...
with open(zone_seq_file, 'rb') as csvfile:
  tapreader = csv.DictReader(csvfile, skipinitialspace=True)
  for row in tapreader:
    node_id    = int(row['N'])
