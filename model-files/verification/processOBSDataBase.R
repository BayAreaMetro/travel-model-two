...
LB_boardings <- other_operators$x[other_operators$tech=="LB" | other_operators$tech=="EB"]
...
other_CR_distribution <- other_CR_distribution[,c("operator", "tourPurpose", "accessMode", "autoSuff", "boards", 
                                                  "trips", "exp_factor", "boardWeight_2015", "tripWeight_2015", "shares_trips", "shares_brdngs")]
all_collapsed_operators <- rbind(unified_collapsed, other_LB_distribution,other_CR_distribution)
...
## multiply trip weights with expansion factor
OBS$expFac <- expansion_factors$exp_factor[match(OBS$operator, expansion_factors$operator)]
OBS$trip_weight2015 <- OBS$trip_weight * OBS$expFac
OBS$board_weight2015 <- OBS$weight * OBS$expFac
...
OBS$usedTotal <- OBS$usedLB+OBS$usedEB+OBS$usedLR+OBS$usedHR+OBS$usedCR
...
# Define AccessEgress [mode used to access or egress transit]
OBS$accessType <- "walk"
