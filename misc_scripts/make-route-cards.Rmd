...
# Remote I/O
```{r remote-io}
transit_dir <- "tests/networkstandard/step2_transit/"
output_file_name <- "transit_for_tableau.csv"
```
...
stops_df <- read_csv(paste0(transit_dir, "stops.txt"), col_types  = "iccddiccc")
shapes_df <- read_csv(paste0(transit_dir, "shapes.txt"), col_types = "iicc")
...
# Reductions
```{r reductions}
nodes_df <- bind_cols(tibble(shst_node_id = nodes_sf$shst_node_id), as_tibble(st_coordinates(nodes_sf))) %>%
  mutate(shst_node_id = paste0(shst_node_id)) %>%
  rename(lng = X, lat = Y)
...
output_shapes_df <- select(routes_df, route_id, agency_id, route_short_name, route_long_name) %>%
  left_join(select(trips_df, route_id, service_id, trip_id, trip_headsign, direction_id, shape_id),
            ., by = c("route_id")) %>%
  left_join(., freq_df, by = c("trip_id")) %>%
  left_join(., select(shapes_df, shape_id, shape_pt_sequence, shape_shst_node_id), by = c("shape_id")) %>%
  left_join(., nodes_df, by = c("shape_shst_node_id" = "shst_node_id")) %>%
