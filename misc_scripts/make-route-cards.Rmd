...
# Reductions
```{r reductions}
nodes_df <- bind_cols(tibble(osm_node_id = nodes_sf$osm_node_id), as_tibble(st_coordinates(nodes_sf))) %>%
  mutate(osm_node_id = paste0(osm_node_id)) %>%
  rename(lng = X, lat = Y)
...
output_df <- select(routes_df, route_id, agency_id, route_short_name, route_long_name) %>%
  ...
  left_join(., select(shapes_df, shape_id, shape_pt_sequence, shape_osm_node_id), by = c("shape_id")) %>%
  left_join(., nodes_df, by = c("shape_osm_node_id" = "osm_node_id"))
