...
import com.pb.mtctm2.abm.reports.SkimBuilder.TripModeChoice;
...
public class OutputTapPairs {
	...
	public void initialize(HashMap<String, String> propertyMap){
        ...
        sequentialMaz = new HashMap<Integer,Integer>();
        for(int i = 1; i<= mgraData.getRowCount();++i){
        	
        	int nonSeqMaz = (int) mgraData.getValueAt(i, "MAZ_ORIGINAL");
        	int seqMaz = (int) mgraData.getValueAt(i,"MAZ");
        	
        	sequentialMaz.put(nonSeqMaz,seqMaz);
        }
	...
	private void createOutputFile(){
        ...
        String headerString = new String(
                ...
                + "localBusIvt,expressBusIvt,hrIvt,frIvt,lrtIvt,crIvt,firstWaitTime,trfWaitTime,fare,mainMode,xfers\n");
	...
	private void run(){
		...
		for(int row = 1; row<=inputDataTable.getRowCount();++row ){
			...
			originMaz = sequentialMaz.get(originMaz);
			destinationMaz = sequentialMaz.get(destinationMaz);
